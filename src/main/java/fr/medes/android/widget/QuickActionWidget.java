...
public abstract class QuickActionWidget extends PopupWindow {
	...
	private void initializeDefault() {
		...
		mArrowOffsetY = mContext.getResources().getDimensionPixelSize(R.dimen.aml__arrow_offset);
	}
	...
	private void showArrow() {
		...
		final View contentView = getContentView();
		final int arrowId = mIsOnTop ? R.id.aml__arrow_down : R.id.aml__arrow_up;
		final View arrow = contentView.findViewById(arrowId);
		final View arrowUp = contentView.findViewById(R.id.aml__arrow_up);
		final View arrowDown = contentView.findViewById(R.id.aml__arrow_down);

		if (arrowId == R.id.aml__arrow_up) {
			arrowUp.setVisibility(View.VISIBLE);
			arrowDown.setVisibility(View.INVISIBLE);
		} else if (arrowId == R.id.aml__arrow_down) {
	...
	private void prepareAnimationStyle() {
		...
		if (arrowPointX <= screenWidth / 4) {
			setAnimationStyle(onTop ? R.style.Medes_Animation_PopUp_Left : R.style.Medes_Animation_PopDown_Left);
		} else if (arrowPointX >= 3 * screenWidth / 4) {
			setAnimationStyle(onTop ? R.style.Medes_Animation_PopUp_Right : R.style.Medes_Animation_PopDown_Right);
		} else {
			setAnimationStyle(onTop ? R.style.Medes_Animation_PopUp_Center : R.style.Medes_Animation_PopDown_Center);
		}
	}
